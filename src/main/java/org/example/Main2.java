...
class BusRouteFinder2 {
    private Connection conn;

    // Initialize database and create tables
    BusRouteFinder2() throws SQLException {
    ...
    private void insertSampleData() throws SQLException {
        ...
        // Insert bus routes
        stmt.execute("INSERT INTO bus_route (name, is_bidirectional) VALUES ('Route 1', 0), ('Route 2', 1)");

        // Insert bus stops
        stmt.execute("INSERT INTO bus_stop (name) VALUES ('Downtown'), ('Park'), ('Mall'), ('Station'), ('Airport')");

        // Insert route plans
        // Route 1: Downtown -> Park -> Mall (one-way)
        ...
        // Route 2: Park -> Station -> Airport (bidirectional)
        stmt.execute("INSERT INTO route_plan (sequence_code, bus_id, bus_stop_id, bus_route_id, route_name) " +
                "VALUES (1, 2, 2, 2, 'Route 2'), (2, 2, 4, 2, 'Route 2'), (3, 2, 5, 2, 'Route 2')");
    ...
    public List<List<String>> findPaths(String startStop, String endStop) throws SQLException {
        ...
        while (!queue.isEmpty()) {
            ...
            // Query for next stops in the forward direction
            PreparedStatement pstmtForward = conn.prepareStatement(
            ...
            pstmtForward.setInt(1, currentStop);
            ResultSet rsForward = pstmtForward.executeQuery();

            while (rsForward.next()) {
                int nextStopId = rsForward.getInt("bus_stop_id");
                String nextStopName = rsForward.getString("name");
            ...
            // Query for next stops in the reverse direction for bidirectional routes
            PreparedStatement pstmtReverse = conn.prepareStatement(
                    "SELECT DISTINCT rp2.bus_stop_id, bs.name " +
                            "FROM route_plan rp1 " +
                            "JOIN route_plan rp2 ON rp1.bus_route_id = rp2.bus_route_id AND rp1.sequence_code > rp2.sequence_code " +
                            "JOIN bus_route br ON rp2.bus_route_id = br.id " +
                            "JOIN bus_stop bs ON rp2.bus_stop_id = bs.id " +
                            "WHERE rp1.bus_stop_id = ? AND br.is_bidirectional = 1");
            pstmtReverse.setInt(1, currentStop);
            ResultSet rsReverse = pstmtReverse.executeQuery();

            while (rsReverse.next()) {
                int nextStopId = rsReverse.getInt("bus_stop_id");
                String nextStopName = rsReverse.getString("name");
    ...
    public static void main(String[] args) {
        try {
            BusRouteFinder2 finder = new BusRouteFinder2();
